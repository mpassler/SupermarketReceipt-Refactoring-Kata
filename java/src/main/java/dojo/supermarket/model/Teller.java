    ...
    private void addDiscountsToReceipt(Map<Product, Double> productsWithCount, Receipt receipt) {
        productsWithCount.keySet().stream()
                ...
                .forEach(product -> {
                    ...
                    calculateAndAddDiscountToReceipt(receipt, product, quantity, offer, quantityAsIntOfferProduct);
                });
    }

    private void calculateAndAddDiscountToReceipt(Receipt receipt, Product product, double quantity, Offer offer, int quantityAsIntOfferProduct) {
